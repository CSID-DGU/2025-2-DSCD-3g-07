...
async def get_walking_route(request: WalkingRouteRequest):
    ...
    try:
        ...
        logger.info(f"[보행자 경로] API 호출 시작: {request.start_name or '출발지'} → {request.end_name or '도착지'}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    logger.error(f"[보행자 경로] Tmap API 오류: {response.status} - {error_text}")
                    raise HTTPException(
                ...
                recalculated_base_time = int(total_distance / tmap_base_speed_mps) if tmap_base_speed_mps > 0 else total_time
                ...
                try:
                    ...
                    logger.info(f"[보행자 경로] 경사도 분석 완료: {elevation_analysis is not None}")
                    if elevation_analysis and elevation_analysis.get('error'):
                        logger.warning(f"[보행자 경로] 경사도 분석 에러: {elevation_analysis['error']}")
                        # 에러가 있어도 횡단보도 정보는 유지
        ...
        raise HTTPException(
            status_code=500,
            detail=f"보행자 경로 검색 중 오류가 발생했습니다: {str(e)}"
        )
