...
class WeatherSpeedModel:
    ...
    def _precip_effect(
        self, T: float, ptype: str, I: float, S: float
    ) -> Tuple[float, float]:
        """강수 효과 계산 (통합)"""
        if ptype == "clear":
            return 1.0, 1.0
    ...
    def predict(
        self, v0_mps: float, weather: WeatherInput, use_smoothing: bool = False
    ) -> SpeedPrediction:
        """
        날씨 조건에서 보행속도 예측
        ...
        f_L_P, f_C_P = self._precip_effect(
            T, weather.ptype, weather.rain_mm_per_h, weather.snow_cm_per_h
        )
    ...
    def predict_from_stride_cadence(
        self, L0: float, C0: float, weather: WeatherInput, use_smoothing: bool = False
    ) -> Dict:
        """
        보폭/보행수 분해 예측
        ...
        return {
            ...
            "weather_coef": result.weather_coeff,
    ...
    def smooth(
        self, prev_coeff: float, new_coeff: float, alpha: Optional[float] = None
    ) -> float:
        """
        계수 스무딩 (EMA)
...
def map_kma_to_weather(
    T: float, PTY: int, RN1: Optional[float] = None, SNO: Optional[float] = None
) -> WeatherInput:
    """
    ...
    return WeatherInput(
        temp_c=T, ptype=ptype, rain_mm_per_h=rain_mm, snow_cm_per_h=snow_cm
    )
