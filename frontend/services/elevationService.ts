...
export async function analyzeRouteSlope(
  ...
  try {
    ...
    try {
      ...
      if (data.factors) {
        console.log(
          `✅ [경사도 분석] 계수 - 사용자: ${data.factors.user_speed_factor.toFixed(3)}, 경사도: ${data.factors.slope_factor.toFixed(3)}, 날씨: ${data.factors.weather_factor.toFixed(3)}, 최종: ${data.factors.final_factor.toFixed(3)}`
        );
      }
 ...
 */
export function getAdjustedWalkingTime(
...
export function getRouteDifficulty(analysis: RouteElevationAnalysis): number {
  const avgMaxSlope =
    analysis.walk_legs_analysis.reduce(
      (sum, leg) => sum + Math.abs(leg.max_slope),
      0
    ) / analysis.walk_legs_analysis.length;

  const timeIncrease =
    (analysis.total_route_time_adjustment / analysis.total_original_walk_time) *
    100;
